...
bool Imu::setup(yarp::os::Property& property) 
{
    ...
    int nrParts = property.find("parts").asFloat64();
    for(int i = 0; i < nrParts; i++)
    {
        std::ostringstream group_i;
        group_i << "part_" << i;
        yarp::os::Bottle &groups = property.findGroup(group_i.str());
   
        yarp::os::Bottle & remoteControlBoardsList = remoteControlBoards.addList(); 
        std::string inputControlBoards;
        inputControlBoards = groups.find("part").asString();
        remoteControlBoardsList.addString("/"+robotName+"/"+inputControlBoards);

        yarp::os::Bottle & axesList = axesNames.addList();
        yarp::os::Bottle *inputJoints;
        inputJoints = groups.find("controlled_joints").asList();
        ...
        inputMoveJoints = groups.find("move_joints").asList();

        for (int movJoint = 0; movJoint < inputMoveJoints->size(); movJoint++)
        {
            movJointsList.addString(inputMoveJoints->get(movJoint).asString());
        }
    }
...
void Imu::run() 
{
    ...
    for (int sensorIndex = 0; sensorIndex < iorientation->getNrOfOrientationSensors(); sensorIndex++)
    {
        ...
        for (int movJoint = 0; movJoint < movJointsList.size(); movJoint++)
        {
            ROBOTTESTINGFRAMEWORK_ASSERT_ERROR_IF_FALSE(ilim->getLimits(model.model().getJointIndex(movJointsList.get(movJoint).toString()), &minLim, &maxLim), Asserter::format("Unable to get limits for joint %s", movJointsList.get(movJoint).toString()));
            moveJoint(model.model().getJointIndex(movJointsList.get(movJoint).toString()), minLim + 5, sensorIndex);
            yarp::os::Time::delay(1.);
            moveJoint(model.model().getJointIndex(movJointsList.get(movJoint).toString()), maxLim - 5, sensorIndex);
            yarp::os::Time::delay(1.);
        }
    }
}
