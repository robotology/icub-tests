                            ...
                            double tol)
{
    ...
    RTF_TEST_CHECK(mat1.rows()==mat2.rows(),"matrix rows do not match");
    RTF_TEST_CHECK(mat1.cols()==mat2.cols(),"matrix cols do not match");
    for(int row=0; row < mat1.rows(); row++ )
    {
        for(int col=0; col < mat1.cols(); col++ )
        {
            RTF_TEST_CHECK(std::fabs(mat1(row,col)-mat2(row,col)) < tol,
