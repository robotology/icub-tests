...
my $str_lgpl2_1_or_later_mat = <<END;
^% iCub Robot Unit Tests \\(Robot Testing Framework\\)
%
% $copyright_iit
%
...
foreach my $filename (@files) {
    ...
    # Skip some text files that do not require license
    if ("$filename" =~ /\.(txt|ini|xml|md|dox)$/) {
    ...
    # Matlab style LGPL2.1+
    if ("$txt" =~ /$str_lgpl2_1_or_later_mat/s) {
        if ("$filename" =~ /\.(m)$/) {
            print "[OK - LGPL2.1+ ($1)] $filename\n";
            $ok++;
        } else {
